...
public class Device : Item
{
    ...
    public void PlaceDevice(SApiObject _apiObj)
    {
        ...
        if (!_apiObj.attributes.HasKeyAndValue("template"))//Rescale according to slot or parent if basic object
        {
            Vector3 scale;
            if (takenSlots.Count > 0)
                scale = new(takenSlots[0].transform.GetChild(0).localScale.x, (float)(double)_apiObj.attributes["height"] / 1000, takenSlots[0].transform.GetChild(0).localScale.z);
            else
                scale = new(transform.parent.GetChild(0).localScale.x, (float)(double)_apiObj.attributes["height"] / 1000, transform.parent.GetChild(0).localScale.z);
        ...
        else
        {
            ...
            height = (float)(double)_apiObj.attributes["height"] / 1000;
        }
        ...
        else
        {
            ...
            if (_apiObj.attributes.ContainsKey("posU"))
                transform.localPosition += new Vector3(0, ((float)(double)_apiObj.attributes["posU"] - 1) * UnitValue.U, 0);
