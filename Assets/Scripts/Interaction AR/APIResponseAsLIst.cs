...
public class APIResponseAsLIst : MonoBehaviour
{
    ...
    public void ToggleParentListAndButtons(GameObject _APIMenu)
    {
        if (_APIMenu.activeSelf)
        {
            _APIMenu.SetActive(false);
        }
        else
        {
            _APIMenu.SetActive(true);
            ApiManager.instance.GetObjectVincent("tenants", null);
        }
    }
}